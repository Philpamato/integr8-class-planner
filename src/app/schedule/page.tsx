...
export default function SchedulePage() {
  ...
  const allPlans = plansDb.all().sort((a, b) => b.weekStart.localeCompare(a.weekStart));
        ...
        <section key={location} className="space-y-4">
          ...
          <div className="space-y-3">
            {blocks.map(blockStart => {
              ...
              const now = new Date();
              const isCurrentBlock = now >= new Date(blockStart) && now <= blockEnd;
                ...
                <div key={blockStart} className={`card overflow-hidden ${isCurrentBlock ? 'ring-2 ring-palestra' : ''}`}>
                  <div className="flex items-center justify-between px-5 py-3 bg-gray-50 border-b border-gray-100">
                    <div className="flex items-center gap-3">
                      ...
                      {isCurrentBlock && (
                        <span className="text-xs bg-palestra text-white px-2 py-0.5 rounded-full font-medium">Current</span>
                      )}
                    </div>
                  ...
                  {blockPlans.length === 0 ? (
                    <div className="px-5 py-4 text-sm text-gray-400 flex items-center justify-between">
                      ...
                      <Link href="/plans/new" className="text-xs text-palestra hover:underline">+ Add plan</Link>
                    </div>
                  ) : (
